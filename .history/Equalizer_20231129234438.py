...
#TODO - CHANGE INTO ONE FUNCTION TO AVOID REPITITION
    ...
    def change_mode_uniform(self):
        ...
        for slider in self.slider_wgts[4:9]:
            slider.setVisible(True)
        for i, widget in enumerate(self.slider_wgts):
            widget.findChild(QtWidgets.QLabel).setText(f"Slider {i}")
            
    def change_mode_instruments(self):
        
        print ("Instrument Mode")
        
        for slider in self.sliders_wgts[4:9]:
            slider.setVisible(False)
        for i, widget in enumerate(self.slider_wgts):
            widget.findChild(QtWidgets.QLabel).setText(f"Instrument {i}")
