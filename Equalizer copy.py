...
class Equalizer(QMainWindow):
    ...
    def set_bands_gains_sliders(self):
        for i in range(10):
            self.sliders[i].setMinimum(0.5)
            self.sliders[i].setMaximum(1.5)
            self.sliders[i].setValue(0.5)
    ...
    def mult_freqs(self, index):
        self.data_modified_fft = self.multiply_fft(
            data = self.data_modified_fft,
            start = self.data_ranges[index][0],
            end = self.data_ranges[index][1],
            index = self.sliders[index].value(),
